...
func (t *bm) generateForFile(file *descriptor.FileDescriptorProto) *plugin.CodeGeneratorResponse_File {
	...
	t.generatePermssionConsts(file)
	t.generatePermssionConstants(file)
	count := 0
...
func (t *bm) generatePermssionConsts(file *descriptor.FileDescriptorProto) {
	t.P()
	t.P("const(")
	...
	for _, service := range file.Service {
		...
		for _, method := range service.Method {
			...
			t.P(` Permission`, naming.MethodName(method), `="Permission`, name, naming.MethodName(method), `"`)
		}
	}
	t.P(")")
}

func (t *bm) generatePermssionConstants(file *descriptor.FileDescriptorProto) {
	t.P()
	t.P("type Permission struct {")
	t.P("  Module string")
	t.P("  Name string")
	t.P("  Url string")
	t.P("  Description string")
	t.P("}")
	t.P("var Perms = []Permission{")
	for _, service := range file.Service {
		name := naming.ServiceName(service)
		for _, method := range service.Method {
			if !t.ShouldGenForMethod(file, service, method) {
			...
			desc := ""
			if err == nil {
				desc = comments.Leading
			}
			//apiInfo := t.GetHttpInfoCached(file, service, method)
			t.P(`  Permission{"`, name, `",Permission`, naming.MethodName(method), `,Path`, name, naming.MethodName(method), `,"`, desc, `"},`)
		}
	}
	t.P("}")
}
...
func (t *bm) generateImports(file *descriptor.FileDescriptorProto) {
	...
	t.P(`	"context"`)
	t.P()
	t.P(`	bm "github.com/gisvr/golib/net/http/blademaster"`)
	t.P(`	"github.com/gisvr/golib/net/http/blademaster/binding"`)
...
func (t *bm) generateBMRoute(
	...
	t.P(`type JSONReaderFunc func(c *bm.Context, data interface{}, err error)`)
	t.P(`func DefaultJSONReader(c *bm.Context, data interface{}, err error) {
	c.JSON(data, err)
}`)
	t.P(`var JF JSONReaderFunc = DefaultJSONReader`)
	...
	for _, method := range service.Method {
		...
		routeName := utils.CamelCase(servName) +
	...
	if isLegacyPkg {
		...
		for _, methInfo := range methList {
			...
			t.P(`e.`, methInfo.apiInfo.HttpMethod, `(Path`, methInfo.routeFuncName, `,`, methInfo.routeFuncName, ` )`)
		}
		t.P(`	}`)
	}
}
